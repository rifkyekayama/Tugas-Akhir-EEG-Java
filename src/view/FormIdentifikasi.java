...
public class FormIdentifikasi extends JPanel {
	...
	public void updateStatusAlat(){
		if(database.getAlatPerekaman() != null){
			int indexAlat = 0;
			switch (database.getAlatPerekaman()) {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getActionCommand().equals("cekKanal2")){
				...
				if(inputDataEEG.showOpenDialog(null) == JFileChooser.APPROVE_OPTION){
					...
					fullName = new String[file1.length];
					...
					if(file1.length > 1){
						...
						for(int i=0; i<file1.length; i++){
							...
							fullName[i] = (String)file1[i].getName();
						}
					}else{
						lblFileDataEEG.setText((String)file1[0].getName());
	...
	class CorePengujian extends SwingWorker<Void, Void>{
		...
		protected Void doInBackground() throws Exception {
			...
			for(i=0; i<dataLatih.pathFile.length; i++){
				...
				try {
					dataLatih.dataEeg = dataLatih.bacaFile(dataLatih.pathFile[i]);
				} catch (IOException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				if(dataLatih.kanal2 == null){
					sinyalKanal1 = dataLatih.segmentasiEEG(dataLatih.dataEeg, dataLatih.kanalToInt(dataLatih.kanal1));
					sinyalFull.add(sinyalKanal1);
					
				}else{
					sinyalKanal1 = dataLatih.segmentasiEEG(dataLatih.dataEeg, dataLatih.kanalToInt(dataLatih.kanal1));
					sinyalKanal2 = dataLatih.segmentasiEEG(dataLatih.dataEeg, dataLatih.kanalToInt(dataLatih.kanal2));
			...
			bobotPelatihan = database.getBobotPelatihan();
			
			System.out.println("bobot pelatihan terbentuk");
			...
			System.out.println("sinyal full terbentuk");
			...
			if(database.getNeuronRileks().get(0)[0].length != database.getBobotPelatihan()[0].length){
				if(database.getStatusWavelet().equals("gelombang")){
					hasilPengujian = lvq.identifikasi(bobotPelatihan[0], bobotPelatihan[1], wavelet.getNeuronPengujian(wavelet.sinyalEEG, database.getSamplingRate(), false));
				}else if(database.getStatusWavelet().equals("filter")){
					hasilPengujian = lvq.identifikasi(bobotPelatihan[0], bobotPelatihan[1], wavelet.getNeuronPengujian(wavelet.sinyalEEG, database.getSamplingRate(), true));
				}
			}else{
				...
				hasilPengujian = lvq.identifikasi(bobotPelatihan[0], bobotPelatihan[1], hasil);
			}
